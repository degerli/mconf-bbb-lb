...
Utils.bbbQueryFromUrl = function(urlObj) {
  return url.format(urlObj).substr(urlObj.pathname.length + 1); // +1 for the '?'
}

// Sends a request with the url 'originalUrl' to 'server'
Utils.requestToServer = function(originalUrl, server, callback) {
  var opt = { url: BigBlueButton.formatBBBUrl(originalUrl, server) }
...
Utils.printMeetings = function(full) {
  Meeting.count(function(err, count) {
    if (count == 0) {
      ...
      Meeting.all(function(err, meetings) {
