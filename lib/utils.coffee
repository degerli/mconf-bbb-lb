...
Utils.gsub = (source, pattern, replacement) ->
  match = undefined
  result = undefined
  return source unless (pattern?) and (replacement?)
  result = ""
...
Utils.unescapeEntities = (str) ->
  ...
  str.replace /&(.+?);/g, (str, ent) ->
    String.fromCharCode (if ent[0] isnt "#" then entityToCode[ent] else (if ent[1] is "x" then parseInt(ent.substr(2), 16) else parseInt(ent.substr(1))))
