...
Nagios.updateServers = function() {
  var opt = { url: config.nagios.url + config.nagios.apiPath, timeout: config.lb.requestTimeout }
  ...
  request(opt, function(error, response, body) {
    ...
    if (Nagios.callback) Nagios.callback(error);
  });
}
...
Nagios.validateServers = function(servers){
  ...
  for (var id in servers) {
    ...
    if (servers[id].services[config.nagios.bbbService] == undefined ||
        service.data == undefined || service.data.trim() == '') {
